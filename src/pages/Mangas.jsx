...
export default function Mangas() {
  ...
  const checks = useSelector((store) => store.mangasnews.checks);
  const text = useSelector((store) => store.mangasnews.text);
  ...
  useEffect(() => {
    ...
    let headers = { headers: { Authorization: `Bearer ${token}` } };
    axios(
      `${apiUrl}/mangas?page=${page}&title=${text}&category_id=${checks.join(
        ","
      )}`,
      headers
    )
        ...
        <form className="w-full flex justify-center my-5">
          {prevPage ? (
            <input
              ...
              type="button"
              value="prev"
              className="w-[100px] h-[50px] pe-[12px] me-[12px] text-xl text-white flex justify-end items-center rounded-[50%_0.5rem_0.5rem_50%] bg-[#4338CA] cursor-pointer hover:drop-shadow-xl"
            />
          ) : (
          ...
          {nextPage ? (
            <input
              ...
              type="button"
              value="next"
              className="w-[100px] h-[50px] ps-[12px] ms-[12px] text-xl text-white flex justify-start items-center rounded-[0.5rem_50%_50%_0.5rem] bg-[#4338CA] cursor-pointer hover:drop-shadow-xl"
            />
          ) : (
            <span className="w-[100px]" />
          )}
